...
当您向Suno API提交音乐视频生成任务时，可以使用 `callBackUrl` 参数设置回调URL。任务完成时，系统将自动向您指定的地址推送结果。
...
生成任务的唯一标识符，与您提交任务时返回的task_id一致

### `data.video_url` (string)
可访问的视频URL，成功时返回，有效期14天
...
- **视频URL有效期为14天**，请及时下载保存到本地存储
- 视频文件通常较大，下载时注意网络稳定性和存储空间
:::
...
<Card
  title="轮询查询结果"
  icon="lucide-radar"
  href="/cn/suno-api/get-music-video-details"
>
 使用获取音乐视频详情端点定期查询任务状态。我们建议每30秒查询一次，
</Card>
